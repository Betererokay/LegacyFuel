...
if Config.EnableHUD then
	...
	Citizen.CreateThread(function()
		while true do
			if displayHud then
				DrawRect(0.128, 0.947, 0.046, 0.03, 0, 0, 0, 100)
				DrawAdvancedText(0.2190 - 0.011, 0.915 - 0.002, 0.002, 0.0328, 0.64, mph, 255, 255, 255, 250, 6, 1)
				DrawAdvancedText(0.2190 - 0.011, 0.925 - 0.002, 0.025, 0.0328, 0.4, "mph", 255, 255, 255, 250, 6, 1)
			else
				Citizen.Wait(750)
			end
    ...
    while true do Citizen.Wait(0)
        ...
        if (vehicle ~= 0) and (class ~= 13) and (GetIsVehicleEngineRunning(vehicle)) then
            if driver and not dead then
				...
				DrawRect(0.1545, 0.897 , 0.007, 0.186, 40, 40, 40, 150)  -- Full fuel bar (black)
                DrawRect(0.1545, 0.897 , barWidth, barHeight, 99, 82, 49, 150)  -- Full fuel bar (black)
                DrawRect(0.1545, 0.897 + (barHeight - fuelHeight) / 2, barWidth, fuelHeight, 208, 125, 4, 255)  -- Curent fuel bar (yellow)
            else
                Wait(500)
            end
        end
